...
const CANDIDATE_FILE = path.join(ROOT, 'forward_candidate_v11.json');
...
async function run(args) {
  ...
  if (dataEnd < candidate.validFrom) throw new Error('no independent V11 forward day is complete yet');
  const endDate = new Date(dataEnd).toISOString().slice(0, 10);
  return runAll({
    candidateFile: CANDIDATE_FILE,
    resultFile: path.join(OUTPUT_DIR, `binance_all_perpetuals_v11_forward_${endDate}.json`),
    outputPrefix: 'binance_all_perpetuals_v11_forward',
