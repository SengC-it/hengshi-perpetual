...
const { chooseFundingCandidate, summarizeFundingPairs } = require('./funding_strategy');
...
function verifyPeriod(name, dates, expected) {
  const trades = parseCsv(path.join(OUTPUT_DIR, `binance_futures_funding_pair_v3_${name}_trades.csv`));
  const equity = parseCsv(path.join(OUTPUT_DIR, `binance_futures_funding_pair_v3_${name}_equity.csv`));
  const actual = summarizeFundingPairs({
    trades,
    equity,
  ...
  const checks = {
    ...
    pricePnl: nearlyEqual(actual.totalPricePnl, expected.totalPricePnl),
    fundingPnl: nearlyEqual(actual.totalFundingPnl, expected.totalFundingPnl),
    fees: nearlyEqual(actual.totalFees, expected.totalFees),
    pairStopRate: nearlyEqual(actual.pairStopRate, expected.pairStopRate)
  };
  return { pass: Object.values(checks).every(Boolean), checks, actual };
}

function validate() {
  const resultFile = path.join(OUTPUT_DIR, 'binance_futures_funding_pair_v3_results.json');
  const result = JSON.parse(fs.readFileSync(resultFile, 'utf8'));
  const development = JSON.parse(fs.readFileSync(path.join(ROOT, 'funding-development-results.json'), 'utf8'));
  const selectedAgain = chooseFundingCandidate(development.candidates);
  ...
  const checks = {
    ...
    evidenceLabelPresent: result.evidenceStatus === 'parameters_frozen_on_2021_2023_before_strategy_level_validation'
  };
  const output = {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'binance_futures_funding_pair_v3_validation.json'), JSON.stringify(output, null, 2));
  return output;
}
