...
export async function fetchJson(path, options = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    let response;
    try {
      response = await fetchImpl(`${FUTURES_API}${path}`, {
...
export async function fetchKlines(symbol, interval, startTime, endTime, options = {}) {
  if (!(endTime >= startTime)) return [];
  const encoded = encodeURIComponent(symbol);
  const rows = await fetchJson(
    `/fapi/v1/klines?symbol=${encoded}&interval=${encodeURIComponent(interval)}&startTime=${Math.trunc(startTime)}&endTime=${Math.trunc(endTime)}&limit=1000`,
    options
  );
...
export async function fetchMarkPrices(symbols, options = {}) {
  const wanted = new Set(symbols);
  if (!wanted.size) return new Map();
  const rows = await fetchJson('/fapi/v1/premiumIndex', options);
  return new Map(rows
    .filter(row => wanted.has(row.symbol))
    .map(row => [row.symbol, Number(row.markPrice)])
    .filter(([, markPrice]) => Number.isFinite(markPrice) && markPrice > 0));
}
