...
export function advancePositionFiveMinute(position, prepared, bars, now, fundingRates = []) {
  const startAfter = Date.parse(position.last_processed_bar);
  const entryTime = Date.parse(position.entry_time);
  const fundingByTime = new Map(fundingRates.map(row => [row.time, row.rate]));
  let stopPrice = Number(position.stop_price);
  ...
  for (const bar of bars) {
    ...
    const currentInterval = Math.floor(bar.openTime / FOUR_HOURS) * FOUR_HOURS;
    if (currentInterval !== intervalStart) {
      intervalStart = currentInterval;
      intervalHigh = -Infinity;
      intervalLow = Infinity;
    }
    intervalHigh = Math.max(intervalHigh, bar.high);
    intervalLow = Math.min(intervalLow, bar.low);
    if (bar.closeTime + 1 === currentInterval + FOUR_HOURS) {
      const index = prepared.indexByTime.get(currentInterval);
      const currentAtr = index == null ? null : prepared.atr[index];
      if (position.trail_atr != null && Number.isFinite(currentAtr)) {
        if (position.side === 1) {
          bestPrice = Math.max(bestPrice, intervalHigh);
          stopPrice = Math.max(stopPrice, bestPrice - position.trail_atr * currentAtr);
        } else {
          bestPrice = Math.min(bestPrice, intervalLow);
          stopPrice = Math.min(stopPrice, bestPrice + position.trail_atr * currentAtr);
        }
      }
      if (position.mean_exit_ema20 && index != null && Number.isFinite(prepared.ema20[index])) {
        if ((position.side === 1 && prepared.bars[index].close >= prepared.ema20[index])
          || (position.side === -1 && prepared.bars[index].close <= prepared.ema20[index])) {
          exitNextOpen = true;
        }
      }
    }
...
export function accountSnapshot(trades, openPositions, preparedBySymbol, currentBarTime, markPrices = new Map()) {
  ...
  for (const position of openPositions) {
    ...
    const price = markPrices.get(position.symbol) ?? (index == null ? Number(position.entry_price) : prepared.bars[index].open);
