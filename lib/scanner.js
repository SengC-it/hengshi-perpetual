...
export async function runShadowScan(options = {}) {
  ...
  try {
    ...
    let [openPositions, exitShadowOpenPositions] = await Promise.all([
      getOpenPositions(STRATEGY.version),
      getOpenPositions(EXIT_SHADOW.version)
    ]);
    const [positionUpdate, exitShadowPositionUpdate] = await Promise.all([
      updateOpenPaperPositions(openPositions, loaded.preparedBySymbol, barTime, requestOptions),
      updateOpenPaperPositions(exitShadowOpenPositions, loaded.preparedBySymbol, barTime, requestOptions)
    ]);
